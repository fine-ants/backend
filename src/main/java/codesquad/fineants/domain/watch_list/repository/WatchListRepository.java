...
public interface WatchListRepository extends JpaRepository<WatchList, Long> {
	...
	@Query(
		"select new codesquad.fineants.domain.watch_list.domain.dto.response.WatchListHasStockResponse(wl.id, wl.name, case when ws.id is not null then true else false end) "
			+
