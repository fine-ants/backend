...
2. 입력 정보를 받은 컨트롤러에서 입력 정보의 유효성을 검증합니다.
3. 컨트롤러에서 서비스로 포트폴리오의 생성을 요청합니다.
4. 서비스는 포트폴리오의 제약 조건*을 검증합니다.
...
5. 유효성 및 제약 조건을 만족하면 서비스는 포트폴리오 객체를 생성합니다.
6. 서비스는 생성한 포트폴리오 객체를 저장소 전달하며 저장을 요청합니다.
7. 저장소는 포트폴리오를 데이터베이스에 저장합니다.

### 4.2 포트폴리오 종목 및 매입 이력 추가

![image](./img/create_holding_purchase-history.png)

1. 회원은 특정한 포트폴리오 페이지로 접속해서 포트폴리오 종목 및 매입 이력 정보를 입력하고 추가를 요청합니다.
2. 입력 정보를 받은 컨트롤러에서 입력 정보의 유효성을 검증합니다.
3. 컨트롤러에서 서비스로 포트폴리오 종목 및 매입 이력 추가를 요청합니다.
4. 서비스는 포트폴리오 종목 및 매입 이력의 제약 조건*을 검증합니다.
    - *포트폴리오의 잔고가 매입 이력을 추가하기에 충분한 금액이 있어야 합니다.
    - *추가하고자 하는 종목이 데이터베이스 내에 존재해야 합니다.
5. 서비스는 포트폴리오 종목 및 매입 이력 객체를 생성합니다.
6. 서비스는 저장소에 포트폴리오 종목 및 매입 이력 데이터 저장을 요청합니다.
7. 저장소는 포트폴리오 종목 및 매입 이력 데이터를 데이터베이스에 저장합니다.

### 4.3 포트폴리오 실시간 정보 조회

포트폴리오 실시간 정보 조회는 포트폴리오와 포트폴리오에 등록되어있는 종목 및 매입이력에 대한 계산 정보들을 실시간으로 조회하는 API입니다.
해당 API는 SSE(Server-Sent-Event) 방식으로 특정 시간 간격으로 데이터를 서버로부터 푸시받습니다.

![image](./img/search_holding_sse.png)

1. 회원은 특정한 포트폴리오 페이지로 접속하여 포트폴리오의 실시간 정보 조회를 요청합니다.
2. 컨트롤러는 포트폴리오 등록번호를 입력받고 서비스에 요청을 위임합니다.
3. 서비스는 포트폴리오 등록번호를 기반으로 SseEmitter 객체를 생성합니다.
    - 생성한 SseEmitter 객체는 Observer 객체 생성시 주입됩니다.
    - 생성한 SseEmitter 객체는 클라이언트에게 HttpResponse로 응답됩니다.
4. 서비스는 주식 장시간 중이라면 PortfolioObservable 컴포넌트를 통하여 Observable 객체를 생성합니다.
    - 생성한 Observable 객체는 5초 간격으로 총 30초 동안 데이터를 발행합니다.
5. Observable 객체는 비동기적으로 수행하면서 구독한 Observer에게 데이터(포트폴리오의 실시간 정보)를 발행합니다.
    - Observable 객체는 실행 중에 서비스에 요청하여 포트폴리오의 실시간 정보를 요청합니다.
6. Observer는 데이터가 발행될 때마다 SseEmitter에게 데이터를 전달하고 이벤트를 생성합니다.
    - PortfolioObserver는 SseEmitter을 주입받는 상태
7. 서버로부터 SseEmitter를 응답받은 클라이언트는 포트폴리오 및 포트폴리오 종목에 대한 실시간 정보를 푸시받습니다.
...
5. 서비스는 각 티커 심볼이 구독 가능*하면 디스패처를 통해서 종목 실시간 체결가 웹소켓 연결을 수행합니다.
    - 티커 심볼이 구독 불가능하다면 REST API 방식으로 현재가를 조회한 다음 레디스에 저장합니다.
    - 웹 소켓 연결된 종목에 대해서 체결가가 발생할때마다 레디스에 체결가를 저장합니다.
    - *최대 구독 가능 개수는 20개
6. 웹소켓 클라이언트는 티커 심볼을 이용하여 한국투자증권과 종목 실시간 체결가 웹소켓 연결하고 실시간으로 종목 체결가를 받습니다.
7. 종목 체결가를 받으면 레디스에 종목에 따른 체결가를 저장합니다.
